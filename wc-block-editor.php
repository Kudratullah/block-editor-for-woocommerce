 ...
 *
 * @wordpress
 * Plugin Name: WooCommerce Block Editor
 * Description: Enable Block Editor (Gutenberg) for WooCommerce Product
 * Plugin URI: https://pixelhive.pro/shop/plugins/wc-block-editor
 * Author: PixelHive
...
if( ! defined( 'PxH_WC_EBE_VERSION' ) ) {
	/**
	 * Plugin Version
	 * @var string
	 * @since 1.0.0
	 */
	define( 'PxH_WC_EBE_VERSION', '1.0.0' );
}
if( ! defined( 'PxH_WC_EBE_FILE' ) ) {
	/**
	 * Absolute path to this plugin main file
	 * @var string
	 * @since 1.0.0
	 */
	define( 'PxH_WC_EBE_FILE', __FILE__ );
}
if( ! defined( 'PxH_WC_EBE_PATH' ) ) {
	/**
	 * Absolute path to this plugin directory without trailing slash
	 * @var string
	 * @since 1.0.0
	 */
	define( 'PxH_WC_EBE_PATH', dirname( PxH_WC_EBE_FILE ) );
}
if( ! defined( 'PxH_WC_EBE_URL' ) ) {
	/**
	 * URL Pointing to this plugin directory with trailing slash
	 * @var string
	 * @since 1.0.0
	 */
	define( 'PxH_WC_EBE_URL', plugins_url( '/', PxH_WC_EBE_FILE ) );
}
...
function PxH_WC_Enable_Block_Editor_Scripts() {
	...
	if ( in_array( $screen_id, array( 'product', 'edit-product' ) ) ) {
		wp_enqueue_script(
			...
			PxH_WC_EBE_URL . 'assets/js/admin' . $suffix . '.js',
			[ 'jquery', 'postbox','wc-admin-product-meta-boxes' ], // make sure it loaded after wp postbox and wc product meta box scripts
			defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? filemtime( PxH_WC_EBE_PATH . '/assets/js/admin.js' ) : PxH_WC_EBE_VERSION,
			true
		);
	}
}
